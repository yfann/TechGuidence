...
+ `Box<dyn Trait>`
    + dyn Trait运行时才知道，大小不固定
        + rust编译时， 对 栈上变量 需要知道大小才能分配空间
            + 不能直接在栈上用`let x: dyn Trait`，大小未知
        + 如果对象在堆上，栈上只存一个 指针，就可以解决大小未知问题
            + `let x: Box<dyn Trait> = Box::new(MyType {});`
    + Box是fat pointer（16 bytes）
        + 存 数据指针（指向堆上对象）
        + 存 vtable 指针（用于动态分发和 drop）
```rust
Box<dyn Trait>
+------------------+
| data pointer      | --> heap object (A 或 B)
+------------------+
| vtable pointer    | --> method table
+------------------+

(*box_trait).foo()
// Rust 取出 data pointer 作为 self
// 取出 vtable pointer 找到对应函数地址
```
...
+ 原子引用计数的智能指针
    + 多个线程 共享同一份数据
    + 通过 原子操作维护引用计数（thread-safe）
    + 当最后一个 Arc 被释放时，数据才会被销毁
+ 多线程共享所有权
    + 不能把一个值move到

+ Arc<T> 只解决“共享所有权”，不解决“可变性”
    + 不能直接改数据
    ```rust
    let x = Arc::new(10);
    // *x += 1; // ❌ 编译错误
...
```rust
// Arc<RwLock<State>>
let r = state.read().unwrap();   // 读
let w = state.write().unwrap();  // 写
```
+ Arc<Atomic*> —— 无锁修改
```rust
// Arc<AtomicUsize>
...
+ 内部可变性（Interior Mutability）
    ...
    + 保持外部不可变引用的安全性
+ 用于引用
    + 同一时间可以有任意多个不可变引用（borrow()）
    + 同一时间只能有一个可变引用（borrow_mut()）
    + 不可变和可变引用不能同时存在
+ 实现编译期可变、不可变引用共存
    + 将报错从编译期推迟到运行时，panic
+ 用于避免编译器误报的情况
+ 解决了 Rust 编译时借用规则无法覆盖的场景
